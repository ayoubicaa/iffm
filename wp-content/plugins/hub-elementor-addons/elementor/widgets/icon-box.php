...
class LD_Icon_Box extends Widget_Base {
	...
	public function get_title() {
		return __( 'Liquid Icon Box', 'hub-elementor-addons' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'image_alt_text',
			[
				'label' => esc_html__( 'Alt Text from attachment?', 'hub-elementor-addons' ),
				'description' => esc_html__( 'If enable, the image alt text will be the image attachment data.', 'hub-elementor-addons' ),
				'type' => Controls_Manager::SWITCHER,
		...
		$this->add_responsive_control(
			'label_offset_top',
			[
				'label' => __( 'Label top offset', 'hub-elementor-addons' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%' ],
				'range' => [
					'px' => [
		...
		$this->add_responsive_control(
			'label_offset_right',
			[
				'label' => __( 'Label right offset', 'hub-elementor-addons' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%' ],
				'range' => [
					'px' => [
		...
		$this->start_controls_tab(
			...
			$this->add_control(
				...
				[
					...
					'type' => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .lqd-iconbox-heading' => 'color: {{VALUE}}',
					],
					'separator' => 'before'
				]
			);
	...
	protected function content_template() {
		...
		function get_toggleable_opts() {
			...
			return 'data-slideelement-onhover="true" data-slideelement-options=\'{ "visibleElement": ".iconbox-icon-wrap, p, ' + settings.title_tag + '", "hiddenElement": ".btn", "alignMid": true, "triggerElement": ".elementor-widget-container" }\'';
		}
		...
		const ib_titleAttrs = {
			'class': [ 'lqd-iconbox-heading', 'elementor-inline-editing', settings.use_inheritance === 'true' ? settings.tag_to_inherite : '' ],
		...
		function get_ib_title() {
			...
			return `<${settings.title_tag} ${view.getRenderAttributeString('ib_titleAttributes')}>${settings.title} ${heading_icon_svg}</${settings.title_tag}>`;
		}
		...
		if( ! empty( $i_type ) ) {
			if( 'image' === $i_type || 'animated' === $i_type ) {
				...
				if( 'svg' === $filetype['ext']) {
					...
					if ( $settings['image_alt_text'] === 'yes' ) {
						$alt_text = get_post_meta(attachment_url_to_postid($icon), '_wp_attachment_image_alt', true);
					} else {
						$alt_text = $settings['title'];
					}
					printf( '<img src="%s" class="lqd-image-icon" alt="%s" />', esc_url( $icon ), esc_attr( $alt_text ) );
				}
			}
			else {
	...
	protected function get_toggleable_opts() {
		...
		$toggleable = $this->get_settings_for_display('toggleable');
		if( 'yes' !== $toggleable ) {
			return;
		}
		...
		return 'data-slideelement-onhover="true" data-slideelement-options=\'{ "visibleElement": ".iconbox-icon-wrap, p, ' . $title_tag . '", "hiddenElement": ".btn", "alignMid": true, "triggerElement": ".elementor-widget-container" }\'';
	...
	protected function get_overlay_link() {
		...
		$link['aria-label'] = $settings['title'];
	...
	protected function get_ib_title() {
		...
		$title_tag = $this->get_settings_for_display('title_tag');
		$tag_to_inherite = $this->get_settings_for_display('use_inheritance') === 'true' ? $this->get_settings_for_display('tag_to_inherite') : '';
		$hover_arrow_svg = '';

		if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
			$class = 'class="lqd-iconbox-heading elementor-inline-editing ' . $tag_to_inherite . '"';
			$inline_edit_attr = 'data-elementor-setting-key="title" data-elementor-inline-editing-toolbar="basic"';
		} else {
			$inline_edit_attr = '';
			$class = 'class="lqd-iconbox-heading ' . $tag_to_inherite . '"';
		}

		if( $this->get_settings_for_display('heading_icon_onhover') === 'yes' ) {
			$hover_arrow_svg = '<svg class="d-inline-block" xmlns="http://www.w3.org/2000/svg" width="12" height="32" viewBox="0 0 12 32"><path fill="currentColor" d="M8.375 16L.437 8.062C-.125 7.5-.125 6.5.438 5.938s1.563-.563 2.126 0l9 9c.562.562.624 1.5.062 2.062l-9.063 9.063c-.312.312-.687.437-1.062.437s-.75-.125-1.063-.438c-.562-.562-.562-1.562 0-2.125z" fill="currentColor"></path></svg>';
		}

		printf( '<%1$s %2$s %3$s>%4$s %5$s</%1$s>', $title_tag, $class, $inline_edit_attr, $title, $hover_arrow_svg );
	}
...
\Elementor\Plugin::instance()->widgets_manager->register( new LD_Icon_Box() );