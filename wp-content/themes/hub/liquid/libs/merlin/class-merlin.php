...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config, array(
				'base_path'            => get_parent_theme_file_path() . '/liquid/libs/',
				'base_url'             => get_parent_theme_file_uri() . '/liquid/libs/',
				'directory'            => 'merlin',
				'merlin_url'           => 'merlin',
	...
	public function admin_page() {
		...
		wp_enqueue_script( 'liquid-admin', liquid()->load_assets( 'js/liquid-admin.min.js' ), array( 'jquery', 'underscore', 'liquid-image-picker' ), true, true );
		//wp_enqueue_script( 'liquid-theme', get_template_directory_uri() .'/assets/js/theme.min.js' );
	...
	protected function step_output() {
		...
		<ul class="merlin__content--steps">
			...
			foreach ( $ouput_steps as $step_key => $step ) :
				...
				<li class="<?php echo esc_attr( $class_attr ); ?>">
					...
					<div class="step-title"><?php echo $step_key === "content" ? $strings['import-header'] : $strings[$step_key.'-header'] ?></div>
	...
	protected function welcome() {
		...
		<div class="merlin__content--transition">
			...
			<img src="<?php echo(liquid()->load_assets( 'img/dashboard/welcome-screen.svg' )) ?>" width="420" />
	...
	protected function license() {
		...
		<div class="merlin__content--transition">
			...
			<p><?php //echo esc_html( sprintf( $paragraph ) ); ?></p>
				...
				<div class="merlin__content--license-key">
					...
					<div class="lqd-dsd-box lqd-dsd-box-solid lqd-dsd-register-box">
						...
						<div class="lqd-dsd-box-head">
							...
							<?php 
								if ( 'valid' == get_option( 'hub_purchase_code_status', false ) ) {
									wp_redirect(admin_url('admin.php?page=liquid-setup&step=plugins'));
								}
							?>
					...
					<p id="license-text" style="display:none"><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>
					<label for="license-key" style="display:none;"><?php echo esc_html( $label ); ?></label>

					<div class="merlin__content--license-key-wrapper" style="display:none">
	...
	protected function plugins() {
		...
		<div class="merlin__content--transition">
			...
			<hr>
			<div class="merlin__builder-buttons">
				<p>Chose your favorite builder</p>
				<div>
					<span class="merlin__builder--elementor"><img src="<?php echo esc_url(get_template_directory_uri() . '/theme/plugins/images/elementor.png'); ?>">Elementor</span>
					<span class="merlin__builder--vc active"><img src="<?php echo esc_url(get_template_directory_uri() . '/theme/plugins/images/bakery-1.jpg'); ?>">WP Bakery</span>
				</div>
			</div>
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'merlin-wp' ); ?>">
										<?php esc_html_e( 'Required', 'merlin-wp' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	protected function content() {
		...
		$this->elementor_update_options();
		?>

			<h1><?php echo esc_html( $header ); ?></h1>
			<p><?php echo esc_html( $paragraph ); ?></p>
			<div class="active-builders">
				<p class="active-builder-wpbakery">Selected Builder: <img src="<?php echo esc_url(get_template_directory_uri() . '/theme/plugins/images/bakery-1.jpg'); ?>"> WPBakery</p>
				<p class="active-builder-elementor">Selected Builder: <img src="<?php echo esc_url(get_template_directory_uri() . '/theme/plugins/images/elementor.png'); ?>"> Elementor</p>
			</div>


		<?php include_once( get_template_directory() . '/liquid/admin/views/liquid-demos.php' ); ?>

		<div class="merlin__content--transition" style="display:none">
		...
		<form style="display:none" action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
	...
	function elementor_update_options(){
		...
		update_option( 'elementor_cpt_support', [ 'page', 'post', 'liquid-header', 'liquid-footer', 'liquid-portfolio', 'liquid-mega-menu', 'ld-product-layout', 'ld-product-sizeguide', 'liquid-sticky-atc' ] );
		liquid_helper()->purge_assets_cache( true );
	}

	function fix_elementor_gap_value() {
		if ( $kit_id = get_option( 'elementor_active_kit' ) ) {

			$kit_data_array = get_post_meta( (int) $kit_id, '_elementor_page_settings', true );
		
			if ( isset( $kit_data_array['space_between_widgets'] ) ) {
				$kit_data_array['space_between_widgets'] = array( 'column' => 0, 'row' => 0 );
