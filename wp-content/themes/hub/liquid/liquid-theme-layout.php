...
class Liquid_Theme_Layout extends Liquid_Base {
	...
	public function body_classes( $classes ) {
		...
		if( class_exists( 'WooCommerce' ) && ( is_product_taxonomy() || is_product_category() || is_shop() ) ) {
	...
	public function start_container( $attributes ) {
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			if (
				liquid_helper()->get_theme_option( 'wc-shop-fullwidth' ) === 'on' &&
	...
	public function setup_sidebar( $sidebar_options ) {
		...
		if( !class_exists( 'ReduxFramework' ) && is_active_sidebar( 'main' ) ||
			...
			// Post Options.
			if ( class_exists( 'Liquid_Elementor_Addons' ) && defined( 'ELEMENTOR_VERSION' ) && is_callable( 'Elementor\Plugin::instance' ) && ( get_the_ID() === liquid_helper()->get_page_id_by_url() ) ) {
				if 	( get_post_type() === 'liquid-header' || get_post_type() === 'liquid-footer' || get_post_type() === 'ld-product-layout' || get_post_type() === 'ld-product-sizeguide' || get_post_type() === 'liquid-sticky-atc' || get_post_type() === 'elementor_library' )  {
					return;
				}
	...
	public function setup_options() {
		...
		elseif ( class_exists( 'WooCommerce' ) && (is_shop() && liquid_helper()->get_theme_option( 'wc-archive-shop-enable' ) === 'yes') ) {
			$sidebars = array(
				'global'    => true,
				'sidebar'   => liquid_helper()->get_theme_option( 'wc-archive-sidebar-one' ),
				'position'  => liquid_helper()->get_theme_option( 'wc-archive-sidebar-position' )
